...
public class FuzzyClustering {
    ...
    public ArrayList<ArrayList<Float>> readFile() throws IOException {
        ArrayList<ArrayList<Float>> tt = new ArrayList<>();
        FileReader fileReader = new FileReader("files/sample.csv");
        BufferedReader csvReader = new BufferedReader(fileReader);
        String row;
        while ((row = csvReader.readLine()) != null) {
            String[] data = row.split(",");
            ArrayList<Float> temp = new ArrayList<>();
            temp.add(Float.valueOf(data[0]));
            temp.add(Float.valueOf(data[1]));
            tt.add(temp);
